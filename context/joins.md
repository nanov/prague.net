...
- **Fallbacks:** any `JoinMany` (design §7.2), any filtered `JoinOne` outside the `SortBounded` → outer-joins shape, and any unfusable inner join send the plan to the replay. In that one shape an unfusable join simply keeps its paired read over the page rows.
- **Inner LeftSym does not fuse by default.** `IJoinResolver.FusedInnerRegroups` is true for `JoinOneLeftSymResolver` alone: its pair set is keyed by the lookup key, so `InnerFanOutContainer` creates the rows **grouped by right** (every left of a bucket together) while a per-left lookup keeps the seed's order — the same set, never the same sequence. A chain containing one therefore replays and stays byte-identical; `FrozenOptions.FuseSymmetricInnerJoins` opts into the fused, reordered form (documented like `ReorderIndexNarrowers`). An *outer* LeftSym join is unaffected — `OuterFanOutContainer` only fills rows the base walk already created, so the order is the base walk's either way. The other three families' inner pair sets are seeded from the candidate set in candidate order, which is the pipeline's seed order, so they fuse with a byte-identical sequence.
