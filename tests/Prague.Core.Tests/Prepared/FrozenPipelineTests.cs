...
public class FrozenPipelineTests {
	...
	public void Pipeline_IsChosen_ForEveryNonCompositeKind_AndNotForTheRest() {
		...
		Assert.Multiple(() => {
			...
			Assert.That(_cache.Prepare<int, PqItem, int>().Match(static a => a, m => m.Case(0, b => b.UseIndex(_byGroup, 1)).Default()).BuildFrozen().Plan.Executor, Is.EqualTo("Pipeline"), "Match (step 4)");
