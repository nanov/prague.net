...
public class FrozenQueryTests {
	...
	[Test]
	public void Replay_IsChosen_ForEveryOtherShape() {
		Assert.Multiple(() => {
			...
			Assert.That(_cache.Prepare().UseIndex(_byCode, new[] { 1042 }).BuildFrozen().Plan.Executor, Is.EqualTo("Replay"), "unique-in alone");
	...
	public void Explain_NamesTheExecutorAndTheOps() {
		...
		Assert.Multiple(() => {
			Assert.That(sorted.Explain(), Does.Contain("executor: Replay"));
	...
	[Test]
	public void Fallback_SortBounded_FrozenEqualsPreparedEqualsEager() {
