...
///   <c>TArms : IOpenMatchArms</c> constraint can refuse an arm after <c>Default</c>.
/// </summary>
...
///   whose tag equals the selected one replays; else the <c>Default</c>; else the step is a no-op
///   (the eager <c>switch</c> with no matching arm). A tag declared twice resolves to its first arm.
///   <para>
...
public static class PreparedQueryBuilderMatchExtensions {
	...
	/// <summary>Records the arm replayed when no <c>Case</c> matched. Closes the chain: no <c>Case</c> binds after it.</summary>
	...
	/// <summary>Replays the first arm whose tag equals <paramref name="selector" />'s result for the execution arguments, else the default, else nothing.</summary>
	...
	/// <summary>Tag-dispatched narrowing inside an <c>If</c> / <c>IfElse</c> branch or a <c>Match</c> arm.</summary>
	...
	/// <summary>Tag-dispatched narrowing inside an <c>Or</c> branch: every arm is narrow-only, as the enclosing branch is.</summary>
	...
	// Case / Default runs its arm lambda against that seed and appends the frozen chain to the arm chain.
